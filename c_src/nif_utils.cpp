...
int get_atom(ErlNifEnv *env, ERL_NIF_TERM term, std::string &var) {
  ...
  if (!enif_get_atom(env, term, &(*(var.begin())), var.size(),
                     ERL_NIF_LATIN1)) {
    return 0;
  }
...
int make(ErlNifEnv *env, const std::vector<uint16_t> &array,
         ERL_NIF_TERM &out) {
  size_t count = array.size();
...
int make(ErlNifEnv *env, const std::vector<uint32_t> &array,
         ERL_NIF_TERM &out) {
  size_t count = array.size();

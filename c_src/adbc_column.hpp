...
struct AdbcColumnType {
  ...
  // only valid if arrow_type is NANOARROW_TYPE_DECIMAL128 or
...
struct AdbcColumnNifTerm {
  ...
  static int from_term(ErlNifEnv *env, ERL_NIF_TERM adbc_column, bool allow_nil,
                       AdbcColumnNifTerm *out);
};

struct AdbcColumnType
adbc_column_type_to_nanoarrow_type(ErlNifEnv *env, ERL_NIF_TERM type_term);
int adbc_column_to_adbc_field(ErlNifEnv *env, ERL_NIF_TERM adbc_column,
                              struct ArrowArray *array_out,
                              struct ArrowSchema *schema_out,
                              struct ArrowError *error_out, unsigned *n_items);
int adbc_column_to_adbc_field(ErlNifEnv *env, struct AdbcColumnNifTerm *column,
                              bool allow_nil, struct ArrowArray *array_out,
                              struct ArrowSchema *schema_out,
                              struct ArrowError *error_out);
int must_be_adbc_column(ErlNifEnv *env, ERL_NIF_TERM adbc_column,
                        ERL_NIF_TERM &struct_name_term, ERL_NIF_TERM &name_term,
                        ERL_NIF_TERM &type_term, ERL_NIF_TERM &metadata_term,
                        ERL_NIF_TERM &data_term, unsigned *n_items);
...
int AdbcColumnNifTerm::from_term(ErlNifEnv *env, ERL_NIF_TERM adbc_column,
  ...
  ERL_NIF_TERM struct_name_term, name_term, type_term, metadata_term, data_term;
  unsigned n_items = 0;
  int ret = must_be_adbc_column(env, adbc_column, struct_name_term, name_term,
                                type_term, metadata_term, data_term, &n_items);
  if (ret != 0) {
    return ret;
  }
...
ERL_NIF_TERM make_adbc_field(ErlNifEnv *env, ERL_NIF_TERM name_term,
  ...
  enif_make_map_from_arrays(env, keys.data(), values.data(),
                            (unsigned)values.size(), &adbc_field);
  return adbc_field;
}
...
ERL_NIF_TERM
make_adbc_column(ErlNifEnv *env, struct ArrowSchema *schema,
                 ERL_NIF_TERM type_term, ERL_NIF_TERM metadata,
  ...
  enif_make_map_from_arrays(env, keys.data(), values.data(),
                            (unsigned)values.size(), &adbc_column);
  return adbc_column;
}

ERL_NIF_TERM make_adbc_column(ErlNifEnv *env, struct ArrowSchema *schema,
                              struct ArrowArray *array, ERL_NIF_TERM name_term,
                              ERL_NIF_TERM type_term, ERL_NIF_TERM metadata,
                              ERL_NIF_TERM data) {
...
ERL_NIF_TERM make_adbc_column(ErlNifEnv *env, struct ArrowSchema *schema,
                              struct ArrowArray *values, const char *name,
                              const char *type, ERL_NIF_TERM metadata,
                              ERL_NIF_TERM data) {
...
int do_get_buffer_datas(ErlNifEnv *env, ERL_NIF_TERM batches_list,
                        size_t element_bytes, struct ArrowArray *array_out,
                        struct ArrowSchema *schema_out,
                        struct ArrowError *error_out);

template <typename T>
int do_get_list_integer(ErlNifEnv *env, ERL_NIF_TERM list,
                        ArrowType nanoarrow_type, struct ArrowArray *array_out,
                        struct ArrowSchema *schema_out,
                        struct ArrowError *error_out) {
  NANOARROW_RETURN_NOT_OK(ArrowSchemaSetType(schema_out, nanoarrow_type));
  return do_get_buffer_datas(env, list, sizeof(T), array_out, schema_out,
                             error_out);
}
...
// per-bit copy.
static int copy_validity_bitmap(const uint8_t *src, int bit_offset,
...
// Parse an %Adbc.BufferData{} struct and bulk-copy data and validity buffers
...
int get_buffer_data(ErlNifEnv *env, ERL_NIF_TERM data_term,
  ...
  if (has_validity) {
    NANOARROW_RETURN_NOT_OK(copy_validity_bitmap(validity_bin.data, bit_offset,
                                                 count, write_array));
  }
...
int do_get_buffer_datas(ErlNifEnv *env, ERL_NIF_TERM data_term,
                        size_t element_bytes, struct ArrowArray *array_out,
                        struct ArrowSchema *schema_out,
                        struct ArrowError *error_out) {
  ...
  NANOARROW_RETURN_NOT_OK(
      ArrowArrayInitFromSchema(write_array, schema_out, error_out));
  ...
  NANOARROW_RETURN_NOT_OK(
      ArrowArrayFinishBuildingDefault(tmp.get(), error_out));
  ArrowArrayMove(tmp.get(), array_out);
  return 0;
}

int do_get_list_decimal(ErlNifEnv *env, ERL_NIF_TERM batches_list,
                        ArrowType nanoarrow_type, int32_t bitwidth,
                        int32_t precision, int32_t scale,
                        struct ArrowArray *array_out,
                        struct ArrowSchema *schema_out,
                        struct ArrowError *error_out) {
  NANOARROW_RETURN_NOT_OK(
      ArrowSchemaSetTypeDecimal(schema_out, nanoarrow_type, precision, scale));
  return do_get_buffer_datas(env, batches_list, bitwidth / 8, array_out,
                             schema_out, error_out);
}

int do_get_dictionary(ErlNifEnv *env, ERL_NIF_TERM type_term,
                      ERL_NIF_TERM batches_list, struct ArrowArray *array_out,
                      struct ArrowSchema *schema_out,
                      struct ArrowError *error_out) {
  ...
  if (!enif_get_tuple(env, type_term, &arity, &tuple_elems) || arity != 3) {
    snprintf(
        error_out->message, sizeof(error_out->message),
  ...
  ret = adbc_column_to_adbc_field(env, &values, true, array_out->dictionary,
                                  schema_out->dictionary, error_out);
  if (ret == 0)
    return ret;
...
int do_get_list_string(ErlNifEnv *env, ERL_NIF_TERM list,
                       ArrowType nanoarrow_type, struct ArrowArray *array_out,
                       struct ArrowSchema *schema_out,
                       struct ArrowError *error_out) {
  NANOARROW_RETURN_NOT_OK(ArrowSchemaSetType(schema_out, nanoarrow_type));
  ...
  // Buffers: 0 = validity, 1 = offsets, 2 = data
  {
    ...
    if (!enif_inspect_binary(env, offsets_term, &offsets_bin))
      return 1;
    if (!enif_inspect_binary(env, data_term, &data_bin))
      return 1;
    ...
    NANOARROW_RETURN_NOT_OK(ArrowBufferAppend(
        ArrowArrayBuffer(write_array, 1), offsets_bin.data, offsets_bin.size));
    ...
    if (has_validity) {
      ErlNifBinary validity_bin;
      int bit_offset = 0;
      if (!enif_inspect_binary(env, validity_term, &validity_bin))
        return 1;
      if (!enif_get_int(env, offset_term, &bit_offset))
        return 1;
      NANOARROW_RETURN_NOT_OK(copy_validity_bitmap(
          validity_bin.data, bit_offset, count, write_array));
    }
...
int do_get_list_boolean(ErlNifEnv *env, ERL_NIF_TERM data_term,
                        ArrowType nanoarrow_type, struct ArrowArray *array_out,
                        struct ArrowSchema *schema_out,
                        struct ArrowError *error_out) {
  NANOARROW_RETURN_NOT_OK(ArrowSchemaSetType(schema_out, nanoarrow_type));
  ...
  if (!enif_inspect_binary(env, data_field, &data_bin))
    return 1;
  ...
  long count = 0;
  if (!enif_get_long(env, size_field, &count))
    return 1;
...
int do_get_list_date(ErlNifEnv *env, ERL_NIF_TERM list,
                     ArrowType nanoarrow_type, struct ArrowArray *array_out,
                     struct ArrowSchema *schema_out,
                     struct ArrowError *error_out) {
  NANOARROW_RETURN_NOT_OK(ArrowSchemaSetType(schema_out, nanoarrow_type));
  return do_get_buffer_datas(env, list,
                             (nanoarrow_type == NANOARROW_TYPE_DATE32) ? 4 : 8,
                             array_out, schema_out, error_out);
}

int do_get_list_time(ErlNifEnv *env, ERL_NIF_TERM list,
                     ArrowType nanoarrow_type, enum ArrowTimeUnit time_unit,
                     struct ArrowArray *array_out,
                     struct ArrowSchema *schema_out,
                     struct ArrowError *error_out) {
  NANOARROW_RETURN_NOT_OK(
      ArrowSchemaSetTypeDateTime(schema_out, nanoarrow_type, time_unit, NULL));
  return do_get_buffer_datas(env, list,
                             (nanoarrow_type == NANOARROW_TYPE_TIME32) ? 4 : 8,
                             array_out, schema_out, error_out);
}

int do_get_list_timestamp(ErlNifEnv *env, ERL_NIF_TERM list,
                          ArrowType nanoarrow_type,
                          enum ArrowTimeUnit time_unit, const char *timezone,
                          struct ArrowArray *array_out,
                          struct ArrowSchema *schema_out,
                          struct ArrowError *error_out) {
  NANOARROW_RETURN_NOT_OK(ArrowSchemaSetTypeDateTime(schema_out, nanoarrow_type,
                                                     time_unit, timezone));
  return do_get_buffer_datas(env, list, 8, array_out, schema_out, error_out);
}

int do_get_list_duration(ErlNifEnv *env, ERL_NIF_TERM list,
                         ArrowType nanoarrow_type, enum ArrowTimeUnit time_unit,
                         struct ArrowArray *array_out,
                         struct ArrowSchema *schema_out,
                         struct ArrowError *error_out) {
...
int do_get_list_interval(ErlNifEnv *env, ERL_NIF_TERM list,
                         ArrowType nanoarrow_type, struct ArrowArray *array_out,
                         struct ArrowSchema *schema_out,
                         struct ArrowError *error_out) {
  NANOARROW_RETURN_NOT_OK(ArrowSchemaSetType(schema_out, nanoarrow_type));
  size_t element_bytes = (nanoarrow_type == NANOARROW_TYPE_INTERVAL_MONTHS) ? 4
                         : (nanoarrow_type == NANOARROW_TYPE_INTERVAL_DAY_TIME)
                             ? 8
                             : 16;
  return do_get_buffer_datas(env, list, element_bytes, array_out, schema_out,
                             error_out);
}

int do_get_struct(ErlNifEnv *env, ERL_NIF_TERM type_term,
  ...
  if (!enif_get_tuple(env, type_term, &arity, &tuple_elems) || arity != 2) {
    snprintf(error_out->message, sizeof(error_out->message),
             "struct type must be {:struct, [fields]}");
    return 1;
  }
  ...
  NANOARROW_RETURN_NOT_OK(ArrowSchemaSetTypeStruct(schema_out, n_children));
  ...
  ERL_NIF_TERM value_head, value_tail = values_term;
  ...
  for (unsigned i = 0; i < n_children; i++) {
    ...
    int ret = adbc_column_to_adbc_field(env, &child_column, true,
                                        child_arrays[i].get(), &child_schema,
                                        error_out);
    if (ret != 0) {
      if (child_schema.release)
        child_schema.release(&child_schema);
      return ret;
    }
    ArrowSchemaMove(&child_schema, schema_out->children[i]);
  }

  // Init array from the fully-populated schema
  NANOARROW_RETURN_NOT_OK(
      ArrowArrayInitFromSchema(array_out, schema_out, error_out));
  ...
  if (has_validity) {
    ...
    if (!enif_inspect_binary(env, validity_term, &validity_bin))
      return 1;
    if (!enif_get_int(env, bit_offset_term, &bit_offset))
      return 1;
    NANOARROW_RETURN_NOT_OK(copy_validity_bitmap(validity_bin.data, bit_offset,
  ...
  NANOARROW_RETURN_NOT_OK(
      ArrowArrayFinishBuildingDefault(array_out, error_out));
  return 0;
}

int do_get_list(ErlNifEnv *env, ERL_NIF_TERM parent_type_term,
                ERL_NIF_TERM list, struct AdbcColumnType *column_type,
                struct ArrowArray *array_out, struct ArrowSchema *schema_out,
                struct ArrowError *error_out) {
  if (column_type == nullptr) {
    enif_snprintf(error_out->message, sizeof(error_out->message),
                  "internal error: column_type is null in do_get_list:%d",
                  __LINE__);
  ...
  if (column_type->arrow_type == NANOARROW_TYPE_FIXED_SIZE_LIST) {
    NANOARROW_RETURN_NOT_OK(ArrowSchemaSetTypeFixedSize(
        schema_out, column_type->arrow_type, column_type->fixed_size));
  } else {
    NANOARROW_RETURN_NOT_OK(
        ArrowSchemaSetType(schema_out, column_type->arrow_type));
  }
  ...
  // data is %{offsets: binary, validity: binary|nil, values: data, offset: int}
  {
    if (!enif_is_map(env, list)) {
      snprintf(error_out->message, sizeof(error_out->message),
               "Expected list data to be a map with offsets, validity, values, "
               "and offset");
      return 1;
    }
    ...
    while (enif_get_list_cell(env, val_tail, &val_head, &val_tail)) {
      ...
      if (!child_initialized) {
        ...
        int ret = adbc_column_to_adbc_field(env, &child_column, true,
                                            extra_array.get(), &extra_schema,
                                            error_out);
        if (extra_schema.release)
          extra_schema.release(&extra_schema);
        if (ret != 0)
          return ret;
    ...
    if (!enif_inspect_binary(env, offsets_term, &offsets_bin)) {
      snprintf(error_out->message, sizeof(error_out->message),
               "Expected offsets to be a binary");
      return 1;
    }
    ...
    size_t offset_elem_size =
        (column_type->arrow_type == NANOARROW_TYPE_LARGE_LIST) ? 8 : 4;
    size_t n_elements = (offsets_bin.size / offset_elem_size) - 1;

    // Copy offsets into the list's offset buffer (buffer index 1).
    // Skip the first offset (0) since ArrowArrayStartAppending already wrote
    // it.
    NANOARROW_RETURN_NOT_OK(ArrowBufferAppend(
    ...
    if (has_validity) {
      NANOARROW_RETURN_NOT_OK(copy_validity_bitmap(
          validity_bin.data, bit_offset, n_elements, array_out));
    }
...
int build_metadata_from_nif(ErlNifEnv *env, ERL_NIF_TERM metadata_term,
  ...
  if (enif_is_map(env, metadata_term)) {
    ...
    while (enif_map_iterator_get_pair(env, &iter, &metadata_key,
      ...
      if (enif_inspect_iolist_as_binary(env, metadata_value, &value_bytes)) {
        ...
        enif_map_iterator_destroy(env, &iter);
        enif_snprintf(error_out->message, sizeof(error_out->message),
                      "cannot get metadata value for key: `%T`", metadata_key);
  ...
  if (enif_is_identical(type_term, kAdbcColumnTypeBool)) {
    ...
    if (enif_is_identical(type_term, kAdbcColumnTypeTime32Seconds)) {
      ...
      ret.time_unit = NANOARROW_TIME_UNIT_SECOND;
                                 ...
                                 kAdbcColumnTypeTime32Milliseconds)) {
      ret.arrow_type = NANOARROW_TYPE_TIME32;
      ret.time_unit = NANOARROW_TIME_UNIT_MILLI;
    } else if (enif_is_identical(type_term,
                                 kAdbcColumnTypeTime64Microseconds)) {
      ...
      ret.time_unit = NANOARROW_TIME_UNIT_SECOND;
                                 ...
                                 kAdbcColumnTypeDurationMilliseconds)) {
      ret.arrow_type = NANOARROW_TYPE_DURATION;
      ret.time_unit = NANOARROW_TIME_UNIT_MILLI;
    } else if (enif_is_identical(type_term,
                                 kAdbcColumnTypeDurationMicroseconds)) {
      ret.arrow_type = NANOARROW_TYPE_DURATION;
      ret.time_unit = NANOARROW_TIME_UNIT_MICRO;
    } else if (enif_is_identical(type_term,
                                 kAdbcColumnTypeDurationNanoseconds)) {
      ret.arrow_type = NANOARROW_TYPE_DURATION;
      ret.time_unit = NANOARROW_TIME_UNIT_NANO;
      ...
      ret.arrow_type = NANOARROW_TYPE_INTERVAL_DAY_TIME;
                                 ...
                                 kAdbcColumnTypeIntervalMonthDayNano)) {
...
int adbc_column_to_adbc_field(ErlNifEnv *env, struct AdbcColumnNifTerm *column,
  ...
  if (column == nullptr) {
    enif_snprintf(error_out->message, sizeof(error_out->message),
                  "internal error, AdbcColumnNifTerm is null");
    return kErrorInternalError;
  }
  ...
  struct ArrowBuffer metadata_buffer{};
  int metadata_ret = build_metadata_from_nif(env, column->metadata_term,
                                             &metadata_buffer, error_out);
  if (metadata_ret) {
  ...
  struct AdbcColumnType column_type =
      adbc_column_type_to_nanoarrow_type(env, column->type_term);
  if (column_type.valid == 0) {
    enif_snprintf(error_out->message, sizeof(error_out->message),
                  "unsupport type `%T` found in adbc_column_to_adbc_field:%d",
                  column->type_term, __LINE__);
    return kErrorBufferUnknownType;
  }

  int ret = kErrorBufferUnknownType;
  if (column_type.arrow_type == NANOARROW_TYPE_BOOL) {
    ret = do_get_list_boolean(env, data_term, column_type.arrow_type, array_out,
                              schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_INT8) {
    ret = do_get_list_integer<int8_t>(env, data_term, NANOARROW_TYPE_INT8,
                                      array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_UINT8) {
    ret = do_get_list_integer<uint8_t>(env, data_term, NANOARROW_TYPE_UINT8,
                                       array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_INT16) {
    ret = do_get_list_integer<int16_t>(env, data_term, NANOARROW_TYPE_INT16,
                                       array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_UINT16) {
    ret = do_get_list_integer<uint16_t>(env, data_term, NANOARROW_TYPE_UINT16,
                                        array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_INT32) {
    ret = do_get_list_integer<int32_t>(env, data_term, NANOARROW_TYPE_INT32,
                                       array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_UINT32) {
    ret = do_get_list_integer<uint32_t>(env, data_term, NANOARROW_TYPE_UINT32,
                                        array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_INT64) {
    ret = do_get_list_integer<int64_t>(env, data_term, NANOARROW_TYPE_INT64,
                                       array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_UINT64) {
    ret = do_get_list_integer<uint64_t>(env, data_term, NANOARROW_TYPE_UINT64,
                                        array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_HALF_FLOAT) {
    NANOARROW_RETURN_NOT_OK(
        ArrowSchemaSetType(schema_out, NANOARROW_TYPE_HALF_FLOAT));
    ret = do_get_buffer_datas(env, data_term, sizeof(uint16_t), array_out,
                              schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_FLOAT) {
    NANOARROW_RETURN_NOT_OK(
        ArrowSchemaSetType(schema_out, NANOARROW_TYPE_FLOAT));
    ret = do_get_buffer_datas(env, data_term, sizeof(float), array_out,
                              schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_DOUBLE) {
    NANOARROW_RETURN_NOT_OK(
        ArrowSchemaSetType(schema_out, NANOARROW_TYPE_DOUBLE));
    ret = do_get_buffer_datas(env, data_term, sizeof(double), array_out,
                              schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_BINARY) {
    ret = do_get_list_string(env, data_term, NANOARROW_TYPE_BINARY, array_out,
                             schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_LARGE_BINARY) {
    ret = do_get_list_string(env, data_term, NANOARROW_TYPE_LARGE_BINARY,
                             array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_STRING) {
    ret = do_get_list_string(env, data_term, NANOARROW_TYPE_STRING, array_out,
                             schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_LARGE_STRING) {
    ret = do_get_list_string(env, data_term, NANOARROW_TYPE_LARGE_STRING,
                             array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_DATE32) {
    ret = do_get_list_date(env, data_term, NANOARROW_TYPE_DATE32, array_out,
                           schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_DATE64) {
    ret = do_get_list_date(env, data_term, NANOARROW_TYPE_DATE64, array_out,
                           schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_LIST) {
    ret = do_get_list(env, column->type_term, data_term, &column_type,
                      array_out, schema_out, error_out);
    ...
    ret = do_get_list(env, column->type_term, data_term, &column_type,
                      array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_TIME32 ||
             column_type.arrow_type == NANOARROW_TYPE_TIME64) {
    ret = do_get_list_time(env, data_term, column_type.arrow_type,
                           column_type.time_unit, array_out, schema_out,
                           error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_DURATION) {
    ret = do_get_list_duration(env, data_term, column_type.arrow_type,
                               column_type.time_unit, array_out, schema_out,
                               error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_TIMESTAMP) {
    ret = do_get_list_timestamp(
        env, data_term, column_type.arrow_type, column_type.time_unit,
        column_type.timezone.c_str(), array_out, schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_INTERVAL_MONTHS) {
    ret = do_get_list_interval(env, data_term, column_type.arrow_type,
                               array_out, schema_out, error_out);
    ...
    NANOARROW_RETURN_NOT_OK(ArrowSchemaSetTypeFixedSize(
        schema_out, column_type.arrow_type, column_type.fixed_size));
    ret = do_get_buffer_datas(env, data_term, column_type.fixed_size, array_out,
                              schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_DECIMAL128 ||
             column_type.arrow_type == NANOARROW_TYPE_DECIMAL256) {
    ret = do_get_list_decimal(env, data_term, column_type.arrow_type,
                              column_type.bits, column_type.precision,
                              column_type.scale, array_out, schema_out,
                              error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_DICTIONARY) {
    ret = do_get_dictionary(env, column->type_term, data_term, array_out,
                            schema_out, error_out);
  } else if (column_type.arrow_type == NANOARROW_TYPE_STRUCT) {
    ret = do_get_struct(env, column->type_term, data_term, array_out,
                        schema_out, error_out);
  }

  if (ret == kErrorBufferUnknownType) {
    enif_snprintf(error_out->message, sizeof(error_out->message),
                  "unsupport type `%T` (arrow_type=%d) found in "
                  "adbc_column_to_adbc_field:%d",
                  column->type_term, column_type.arrow_type, __LINE__);
  }
  return ret;
}
...
int adbc_column_to_adbc_field(ErlNifEnv *env, ERL_NIF_TERM adbc_column,
  ...
  ret = adbc_column_to_adbc_field(env, &column, allow_nil, array_out,
                                  schema_out, error_out);
  if (n_items) {
...
// non-zero return value indicating errors
int adbc_column_to_arrow_type_struct(ErlNifEnv *env, ERL_NIF_TERM values,
                                     struct ArrowArray *array_out,
                                     struct ArrowSchema *schema_out,
                                     struct ArrowError *error_out) {
  unsigned n_items = 0;
  ...
  NANOARROW_RETURN_NOT_OK(
      ArrowArrayAllocateChildren(array_out, static_cast<int64_t>(n_items)));
  array_out->length = 1;
    ...
    if (enif_is_map(env, head)) {
      ...
      if (enif_is_ref(env, data_term)) {
        snprintf(
            ...
            "Please call `Adbc.Result.materialize/1` or "
            "`Adbc.Column.materialize/1` first.");
        return 1;
      }

      ArrowSchemaInit(schema_i);
      unsigned n_items = 0;
      int ret = adbc_column_to_adbc_field(env, head, false, child_i, schema_i,
                                          error_out, &n_items);
      ...
      case kErrorBufferWrongStruct:
        snprintf(error_out->message, sizeof(error_out->message),
                 "Expected `%%Adbc.Column{}` or primitive data types.");
        return 1;
      ...
      case kErrorBufferDataIsNotAList:
        snprintf(error_out->message, sizeof(error_out->message),
                 "Expected the `data` field of `Adbc.Column` to be a list of "
                 "values.");
        return 1;
      case kErrorBufferDataIsNotAMap:
        snprintf(error_out->message, sizeof(error_out->message),
                 "Expected the `data` field of dictionary `Adbc.Column` to be "
                 "a map.");
        return 1;
      ...
      if (enif_get_int64(env, head, &i64)) {
        NANOARROW_RETURN_NOT_OK(
            ArrowSchemaSetType(schema_i, NANOARROW_TYPE_INT64));
        NANOARROW_RETURN_NOT_OK(ArrowSchemaSetName(schema_i, ""));
        NANOARROW_RETURN_NOT_OK(
            ArrowArrayInitFromSchema(child_i, schema_i, error_out));
        ...
        NANOARROW_RETURN_NOT_OK(
            ArrowSchemaSetType(schema_i, NANOARROW_TYPE_DOUBLE));
        NANOARROW_RETURN_NOT_OK(ArrowSchemaSetName(schema_i, ""));
        NANOARROW_RETURN_NOT_OK(
            ArrowArrayInitFromSchema(child_i, schema_i, error_out));
        NANOARROW_RETURN_NOT_OK(ArrowArrayStartAppending(child_i));
        ...
        if (enif_is_identical(head, kAtomTrue)) {
          ...
          enif_snprintf(error_out->message, sizeof(error_out->message),
                        "atom `:%T` is not supported yet.", head);
          return 1;
        }
        ...
        enif_snprintf(
            error_out->message, sizeof(error_out->message),
            "unsupported parameter `%T` in adbc_column_to_arrow_type_struct:%d",
            head, __LINE__);
        return 1;
      }
    }
  ...
  NANOARROW_RETURN_NOT_OK(ArrowArrayFinishBuilding(
      array_out, NANOARROW_VALIDATION_LEVEL_FULL, error_out));

...
template <typename T>
int make_f64_list_from_c_array(ErlNifEnv *env, size_t count, T *data,
                               ERL_NIF_TERM &out) {
  if (count == 0) {
...
template <typename T>
int make_i64_list_from_c_array(ErlNifEnv *env, size_t count, T *data,
                               ERL_NIF_TERM &out) {
  if (count == 0) {
    out = enif_make_list_from_array(env, nullptr, 0);
    return 0;
...
template <typename T>
int make_u64_list_from_c_array(ErlNifEnv *env, size_t count, T *data,
                               ERL_NIF_TERM &out) {
  if (count == 0) {
    out = enif_make_list_from_array(env, nullptr, 0);
    return 0;
...
template <typename T>
int make_i32_list_from_c_array(ErlNifEnv *env, size_t count, T *data,
                               ERL_NIF_TERM &out) {
  if (count == 0) {
    out = enif_make_list_from_array(env, nullptr, 0);
    return 0;
...
template <typename T>
int make_u32_list_from_c_array(ErlNifEnv *env, size_t count, T *data,
                               ERL_NIF_TERM &out) {
  if (count == 0) {
    out = enif_make_list_from_array(env, nullptr, 0);
    return 0;
...
#ifndef OS_WIN
int make(ErlNifEnv *env, const std::vector<unsigned long long> &array,
         ERL_NIF_TERM &out);
#endif
...
int make(ErlNifEnv *env, const std::vector<long long> &array,
         ERL_NIF_TERM &out);
...
int make(ErlNifEnv *env, const std::vector<std::string> &array,
         ERL_NIF_TERM &out);
int make(ErlNifEnv *env, const std::vector<const std::string *> &array,
         ERL_NIF_TERM &out);
...
int make(ErlNifEnv *env, const std::map<Key, Value> &map, ERL_NIF_TERM &out,
  ...
  ERL_NIF_TERM *values =
      (ERL_NIF_TERM *)enif_alloc(sizeof(ERL_NIF_TERM) * size);
  if (!values) {
    enif_free(keys);
    return 1;
  }
...
template <typename Key, typename Value>
int make(ErlNifEnv *env, const std::vector<std::map<Key, Value>> &array,
         ERL_NIF_TERM &out, bool atom_key) {

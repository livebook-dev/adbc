...
defmodule Adbc.Result do
  ...
  """
  def to_py(%Adbc.Result{data: batches}) when is_list(batches) do
    # Each batch is a list of columns. Flatten into a single batch
    # by taking the first batch (most results are single-batch).
    columns =
      case batches do
        [single_batch] when is_list(single_batch) -> single_batch
        [] -> []
        _ -> List.flatten(batches)
      end
